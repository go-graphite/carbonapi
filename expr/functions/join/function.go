...
func (f *join) Do(ctx context.Context, eval interfaces.Evaluator, e parser.Expr, from, until int64, values map[parser.MetricRequest][]*types.MetricData) (results []*types.MetricData, err error) {
	seriesA, err := helper.GetSeriesArg(ctx, eval, e.Arg(0), from, until, values)
	if err != nil {
		return nil, err
	}
	seriesB, err := helper.GetSeriesArg(ctx, eval, e.Arg(1), from, until, values)
	if err != nil {
		return nil, err
	}
