...
func DateParamToEpoch(s string, qtz string, d int64, defaultTimeZone *time.Location) uint32 {
	...
	if s[0] == '-' {
		...
		return uint32(timeNow().Add(time.Duration(offset) * time.Second).Unix())
	}
	...
	case "midnight", "noon", "teatime":
		...
		return uint32(dt.Unix())
	}

	sint, err := strconv.Atoi(s)
	// need to check that len(s) > 8 to avoid turning 20060102 into seconds
	if err == nil && len(s) > 8 {
		return uint32(sint) // We got a timestamp so returning it
	}
